  ...
  download: async (link, format, type = 'video') => {
    if (!link) {
      return {
        status: false,
        code: 400,
        error: "What are you downloading? Enter the link, idiot."
      };
    }
    ...
    if (!ogmp3.isUrl(link)) {
      return {
        status: false,
        code: 400,
        error: "That link is invalid. Put a link to a valid YouTube video, idiots. 🗿"
      };
    }
    ...
    if (!valid_fmt.includes(format)) {
      return {
        status: false,
        code: 400,
        error: `Format ${format} is not valid for ${type} But you can choose one of these: ${valid_fmt.join(', ')}`
      };
    }
    ...
    if (!id) {
      return {
        status: false,
        code: 400,
        error: "Where the hell is the video link? I can't extract it, damn it!"
      };
    }
    ...
    try {
      ...
      while (retries < maxRetries) {
        ...
        if (data.le) {
          return {
            status: false,
            code: 400,
            error: "The video is way too long, dude. The maximum is 3 hours, you can't go over that, got it?? 👍🏻"
          };
        }
        ...
        if (data.i === "blacklisted") {
          ...
          return {
            status: false,
            code: 429,
            error: `Daily download limit (${limit}) If you've reached the target, please try again later..`
          };
        }
        ...
        if (data.e || data.i === "invalid") {
          return {
            status: false,
            code: 400,
            error: "The video doesn't exist, idiot. I don't know if it was taken down or if YouTube restricted it... I have no idea. 🤷🏻"
          };
        }
      ...
      return {
        status: false,
        code: 500,
        error: "I'm exhausted, idiot... I've already tried submitting the request several times and it still isn't working, so I'll leave the request for later, see you later! 😂"
      };
