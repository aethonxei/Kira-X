...
export default async function (m, conn = { user: {} }) {
    ...
    let user = (global.db && global.db.data && global.db.data.users) ? global.db.data.users[m.sender] : null;
    // Guard against missing conn.user or jid
    let meJid = conn.user && conn.user.jid ? conn.user.jid : '0@s.whatsapp.net';
    let me = PhoneNumber('+' + meJid.replace('@s.whatsapp.net', '')).getNumber('international');

    let oraAttuale = new Date();
    let oraColombia = oraAttuale.toLocaleString('it-IT', { hour: '2-digit', minute: '2-digit', second: '2-digit' });

    let chatName = chat ? (m.isGroup ? 'Group: ' + chat : 'Private chat: ' + chat) : '';
    console.log(`╭────────────────────────────────···
