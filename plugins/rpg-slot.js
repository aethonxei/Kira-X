...
const handler = async (m, { args, usedPrefix, command, conn }) => {
  const fa = `${emoji} Please enter the amount you want to bet.`.trim();
  if (!args[0] || isNaN(args[0]) || parseInt(args[0]) <= 0) throw fa;
  
  const apuesta = parseInt(args[0]);
  const users = global.db.data.users[m.sender];
  const time = users.lastslot + 10000;
  if (new Date() - users.lastslot < 10000) throw `${emoji2} You must wait ${msToTime(time - new Date())} before using #slot again.`;
  if (apuesta < 100) throw `${emoji2} The minimum bet is 100 XP.`;
  if (users.exp < apuesta) {
    throw `${emoji2} Your XP is not enough to contribute that amount.`;
  }
...
────────`;
  ...
  let end;
  if (x[0] === y[0] && y[0] === z[0]) {
    end = `${emoji} You won! 🎁 +${apuesta + apuesta} XP.`;
    users.exp += apuesta;
  } else if (x[0] === y[0] || x[0] === z[0] || y[0] === z[0]) {
    end = `${emoji2} You almost made it! Keep trying! = *Take +10 XP*`;
    users.exp += 10;
  } else {
    end = `${emoji4} You lost -${apuesta} XP`;
    users.exp -= apuesta;
  }
