...
const handler = async (m, { conn, usedPrefix, command, text, args }) => {
  ...
  if (!efecto) {
    ...
    let responseText = `${emoji} You haven't entered an effect, please enter a voice effect.\n\n*${emoji2} Choose one of the following effects:*\n`;

    for (let i = 0, count = 0; count < 100 && i < voiceList.resultado.length; i++) {
      const entry = voiceList.resultado[i];
      if (entry.ID.length <= 20) {
        responseText += `*◉ ${usedPrefix + command} ${entry.ID} your-text-here*\n`;
        count++;
      }
    }
  ...
  if (!efectoValido) return conn.sendMessage(m.chat, { text: `${emoji2} The effect provided is not listed, use ${usedPrefix + command} to see the list of effects.` }, { quoted: m });

  if (!texto) return conn.sendMessage(m.chat, {text: `${emoji} Enter the text you want to convert to audio.\n\n> *${emoji2} Example: ${usedPrefix + command} ${efecto} Hello, this is an example of using the command.*`}, {quoted: m});
...
async function getVoiceList() {
  ...
  try {
    ...
    return { resultado: simplifiedList ? simplifiedList : `${msm} Error, no response was received from the API.` };
  } catch (error) {
    console.error('Error:', error);
    return { resultado: `${msm} Error, no response was received from the API.` };
throw error;
  }
}
...
async function makeTTSRequest(texto, efecto) {
  ...
  try {
    ...
    return { resultado: url ? url : `${emoji2} URL not found in response.` };
  } catch (error) {
    console.error('Error:', error);
    return { resultado: `${msm} Error, no response was received from the API.` };
  }
