const handler = async (m, { isPrems, conn }) => {
  ...
  if (Date.now() < timeToNextCofre) {
    ...
    const mensajeEspera = `${emoji3} Have you already claimed your chest?\n⏰️ Return in: *${msToTime(tiempoRestante)}* to claim again.`;
...
╰━━━━━━━━━━━━⬣`;
  ...
  try {
    ...
    throw `${msm} An error occurred while sending the chest.`;
  }
};

handler.help = ['chest'];
handler.tags = ['rpg'];
handler.command = ['chest'];
handler.level = 5;
handler.group = true;
