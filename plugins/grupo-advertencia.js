...
who = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text; 
  ...
  const dReason = 'For no reason';
  const msgtext = text || dReason 
  const sdms = msgtext.replace(/@\d+-?\d* /g, '');
  const warntext = `${emoji} Tag a person or reply to a group message to warn the user.`;
  if (!who) {
...
return
  ...
  user.warn += 1;
  await m.reply(`${user.warn == 1 ? `*@${who.split`@`[0]}*` : `*@${who.split`@`[0]}*`} received a warning in this group!.\nReason: ${sdms}\n*Warnings: ${user.warn}/3*`, null, { mentions: [who] },
  );
  if (user.warn >= 3) {
    user.warn = 0;
    await m.reply(`${emoji} I warned you several times!!!.\n*@${who.split`@`[0]}* You exceeded the *3* warnings, now you will be eliminated..`, null, { mentions: [who] },
    );
