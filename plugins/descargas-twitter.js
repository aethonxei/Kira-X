...
const handler = async (m, { conn, text, usedPrefix, command, args }) => {
    if (!args || !args[0]) return conn.reply(m.chat, `${emoji} You forgot the link to a Twitter image/video.`, m);
    if (enviando) return; 
    enviando = true;
    ...
    try {
        ...
        const caption = res.caption ? res.caption : `${emoji} Here's your Twitter video :3.`;
