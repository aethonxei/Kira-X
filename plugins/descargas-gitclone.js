...
let handler = async (m, { args, usedPrefix, command }) => {
  if (!args[0]) {
    return conn.reply(m.chat, `${emoji} Please enter the URL of a GitHub repository you wish to download.`, m)
  }
  if (!regex.test(args[0])) {
    return conn.reply(m.chat, `${emoji2} Verify that the *URL* is from GitHub`, m).then(_ => m.react(error))
  }
