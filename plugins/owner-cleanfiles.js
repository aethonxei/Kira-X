...
const _0xcleanTempFiles = async () => {
    ...
    if (_0xallTempFiles.length === 0) {
        return `${emoji2} No temporary files were found to delete.`;
    }
    ...
    return `${emoji} Removed ${_0xallTempFiles.length} temporary files correctly.`;
};

let _0xhandler = async (m, { conn, isOwner }) => {
    if (!isOwner) return conn.reply(m.chat, `${emoji} Only the owner can use this command.`, m);
