...
if (!db.data.chats[m.chat].nsfw && m.isGroup) {
    ...
    if (!args[0]) {
        return conn.reply(m.chat, `${emoji} Please send an Xvideos link to download the video.`, m);
    }

    try {
        conn.reply(m.chat, `${emoji2} The video is being processed, please wait a moment...\n\n- Sending time depends on the weight and length of the video.`, m);
