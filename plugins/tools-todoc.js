...
if (!m.quoted) return conn.reply(m.chat,  `${emoji} Tag the *Video or Audio* you want to convert into a document.`, m)
if(!text) return conn.reply(m.chat, `${emoji2} Enter the name to save the document.`, m)
if (!/audio|video/.test(mime)) return conn.reply(m.chat,  `${emoji} Tag the *Video or Audio* you want to convert into a document.`, m)
