...
if (!db.data.chats[m.chat].nsfw && m.isGroup) {
    ...
    let str;
    if (m.mentionedJid.length > 0) {
        str = `\`${name2}\` *fucked* \`${name || who}\`.`;
    } else if (m.quoted) {
        str = `\`${name2}\` *is fucking* \`${name || who}\`.`;
    } else {
        str = `\`${name2}\` *is making scissors! >.<*`.trim();
    }
...
handler.help = ['lesbians/lesbian @tag'];
handler.tags = ['nsfw'];
handler.command = ['yuri','lesbians','lesbian'];
handler.group = true;
