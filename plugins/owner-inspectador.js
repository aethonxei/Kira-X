...
case isCommand1:
let inviteCode
if (!text) return await conn.reply(m.chat, `${emoji} Enter the group/community or channel link.`, m)
const MetadataGroupInfo = async (res, isInviteInfo = false) => {
let nameCommunity = "does not belong to any Community"
let groupPicture = "It could not be obtained"
...
`🔰 *Total users:*\n${res.size || "Amount not found"}\n\n` +
`✨ *Advanced information* ✨\n\n🔎 *Community linked to the group:*\n${res.isCommunity ? "This group is a chat room for announcements." : `${res.linkedParent ? "`Id:` " + res.linkedParent : "This group"} ${nameCommunity}`}\n\n` +
`⚠️ *Restrictions:* ${res.restrict ? "✅" : "❌"}\n` +
`📢 *Advertisements:* ${res.announce ? "✅" : "❌"}\n` +
`🏘️ *Is it a community??:* ${res.isCommunity ? "✅" : "❌"}\n` +
`📯 *Is this a community announcement?:* ${res.isCommunityAnnounce ? "✅" : "❌"}\n` +
`🤝 *It has member approval:* ${res.joinApprovalMode ? "✅" : "❌"}\n` +
`🆕 *You can add future members:* ${res.memberAddMode ? "✅" : "❌"}\n\n` 
return caption.trim()
}
...
switch (key) {
case "subscribers":
return value ? value.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".") : "There are no subscribers."
case "creation_time":
case "nameTime":
...
case "BASIC": return "Basic permitted reactions"
...
case "mute":
switch (value) {
case "ON": return "Silenced"
case "OFF": return "Not silenced"
...
.replace("State", "📌 State")
.replace("Creation Time", "📅 Date of creation")
.replace("Name Time", "✏️ Date of name modification")
.replace("Name", "🏷️ Name")
.replace("Description Time", "📝 Date of modification of the description")
