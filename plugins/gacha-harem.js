...
let handler = async (m, { conn, args }) => {
    try {
        ...
        if (userCharacters.length === 0) {
            await conn.reply(m.chat, '❀ You have no characters claimed in your harem.', m)
            return
        }
        ...
        if (page < 1 || page > totalPages) {
            await conn.reply(m.chat, `❀ Invalid page. There are a total of *${totalPages}* pages.`, m)
            return
        }

        let message = `✿ Claimed characters ✿\n`
        message += `⌦ User: @${userId.split('@')[0]}\n`
        message += `♡ Characters: *(${totalCharacters}):*\n\n`
        ...
        await conn.reply(m.chat, `✘ Error loading harem: ${error.message}`, m)
    }
}
