...
let handler = async (m, { conn, args }) => {
  ...
  let text = `◢✿ *Top users with the most experience* ✿◤\n\n`;

  for (let i = startIndex; i < endIndex; i++) {
    let u = users[i];
    let name = await conn.getName(u.jid) || 'Unknown';
    text += `✰ ${i + 1} » *${name}*\n`;
    text += `  ❖ XP » *${u.exp.toLocaleString()}*  ❖ LVL » *${u.level}*\n`;
  }

  text += `\n> • Page *${page}* of *${totalPages}*`;
  if (page < totalPages) text += `\n> To view the next page » *#leaderboard ${page + 1}*`;
