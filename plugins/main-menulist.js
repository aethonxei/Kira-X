...
const defaultMenu = {
  before: `Hello %name ${ucapan()} , I am Kira-X bot
...
let handler = async (m, { conn, usedPrefix: _p, __dirname }) => {
  try {
    ...
    const imageUrl = 'https://files.catbox.moe/gegwez.jpg';
...
id: `${_p}menubusquedas`
    ...
    const interactiveMessage = {
      ...
      footer: { text: "𖥔🍎ᩚ⋆｡˚ ꒰❄︎ ʸᵃᵍᵃᵐⁱ-ˡⁱᵍʰᵗ" },
      nativeFlowMessage: {
        buttons: [
          {
            name: "single_select",
            buttonParamsJson: JSON.stringify({
              title: "❀⃘⃛͜ ۪۪۪݃𓉘᳟ี ⃞̸͢𑁃 ̚𓉝᳟ี𝐁𝐎𝐓 𝐌𝐄𝐍𝐔❀⃘⃛͜ ۪۪۪݃",
              sections: sections
            })
          }
        ],
    ...
    conn.reply(m.chat, `❀⃘⃛͜ ۪۪۪݃ My apologies, the menu could not be loaded.\n\n✐ *Reason:* ${e}`, m);
    throw e;
  }
};
...
function ucapan() {
  ...
  let res = "Good night🌙";
  if (time >= 5 && time < 12) res = "Good morning☀️";
  else if (time >= 12 && time < 18) res = "Good afternoon🌤️";
