...
var handler = async (m, { conn, text }) => {
    ...
    if (!who) return conn.reply(m.chat, `${emoji} Please enter a user's tag or reply to a message.`, m);

    if (!userName) {
        userName = text || 'Unknown user';
    }

    let start = `🧑‍💻 *Hacking started*...`;
...
handler.help = ['hack'];
handler.tags = ['fun'];
handler.command = ['hack', 'hacked', 'doxeo'];
